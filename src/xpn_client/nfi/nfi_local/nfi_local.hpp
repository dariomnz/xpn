...
namespace XPN
{
    ...
    {
    public:
        nfi_local(const xpn_parser &parser) : nfi_server(parser) {}
    public:
        // Operations 
        ...
        int nfi_readdir     (const std::string_view &path, xpn_fh &fh, struct ::dirent &entry) override;
        int nfi_closedir    (const std::string_view &path, const xpn_fh &fh) override;
